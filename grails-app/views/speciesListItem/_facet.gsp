    ...
    <ul>
        ...
        <g:while test="${i < 4 && i < values.size()}">
            ...
            <g:if test="${isProperty}">
                <li><g:link action="list" id="${params.id}" params="${[fq:sl.buildFqList(fqs:fqs, fq:"kvp ${arr[0]}:${arr[1]}"), max:params.max]}">${arr[2]?:arr[1]}</g:link> (${arr[3]})</li>
            </g:if>
            <g:else>
                <li><g:link action="list" id="${params.id}" params="${[fq:sl.buildFqList(fqs:fqs, fq:"${key}:${arr[0]}"), max:params.max]}">${arr[0]}</g:link> (${arr[1]})</li>
            </g:else>
            <% i++ %>
        </g:while>
        <g:if test="${values.size() > 4}">
            <div class="showHide">
                ...
                <div id="${sl.facetAsId(key:key, prefix:"div")}" class="modal hide " tabindex="-1" role="dialog" aria-labelledby="multipleFacetsLabel" aria-hidden="true"><!-- BS modal div -->
                    ...
                    <div class="modal-body">
                        <table class="table table-bordered table-condensed table-striped scrollTable" style="width:100%;">
                            ...
                            <g:each in="${values}" var="arr">
                                <tr>
                                    <g:if test="${isProperty}">
                                        <td><g:link action="list" id="${params.id}" params="${[fq:sl.buildFqList(fqs:fqs, fq:"kvp ${arr[0]}:${arr[1]}"), max:params.max]}">${arr[2]?:arr[1]}</g:link></td>
                                        <td style="text-align: right; border-right-style: none;">${arr[3]}</td>
                                    </g:if>
                                    <g:else>
                                        <td><g:link action="list" id="${params.id}" params="${[fq:sl.buildFqList(fqs:fqs, fq:"${key}:${arr[0]}"), max:params.max]}">${arr[0]}</g:link></td>
