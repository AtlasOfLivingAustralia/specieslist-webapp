...
class SpeciesListController {
    ...
    def uploadList(){
        ...
        if(formParams.speciesListName && formParams.headers && formParams.rawData){
            ...
            println("Loading species list " + formParams.speciesListName)
...
//        if (params.rawData && params.speciesListName && params.header){
//            def drURL = helperService.addDataResourceForList(params.speciesListTitle)
//            def druid = drURL.toString().substring(drURL.lastIndexOf('/') +1)
//
//        }
     ...
     *
     * TODO work out whether this needs to be done offline with an email sent after the process is finished
     */
    def submitList(){
...
class UserDetailsCommand{
    ...
    String user= "Get from";
    String email = "natasha.carter@csiro";
