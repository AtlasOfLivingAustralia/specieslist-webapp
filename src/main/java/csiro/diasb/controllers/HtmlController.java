 ...
 * @version $Id:  HtmlController.java 697 2009-08-21 11:09:45Z oak021 $
 */
...
/**
 * Controls the display of HtmlContentModel objects.
 *
 ...
 */
@Results({
    @Result(name="success", type="redirectAction", params = {"actionName" , "html"})
})
public class HtmlController extends ActionSupport {
...
/**
 * List of all current html files in the Fedora repository (made if no
 ...
 */
 ...
 //RELS-EXT properties
  ...
  private final boolean DEBUG_MSG = true;
   ...
   /**
   * Entry point to the controller from /AlaHarvester/html/<pid>
   * It displays a table of the html file's properties

   * @return Forwards results to the html-show web page
   *
   * @since v0.1
   */
...
//next look for RELS-EXT properties
   ...
   //now look for attributed properties
	...
	} // End of `HtmlController.show` method.

   /**
   * Entry point to the controller from /AlaHarvester/html/
   * It displays a list of all html files in the repository

   * @return Forwards results to the html-index web page
   *
   * @since v0.1
   */
   public HttpHeaders index() {
        ...
        try {
            sr = (ArrayList<SearchResult>) PseudoRepository.doInitIndex("hasModel", "ala:HtmlPageContentModel", null);
        } catch (Exception ex) {
