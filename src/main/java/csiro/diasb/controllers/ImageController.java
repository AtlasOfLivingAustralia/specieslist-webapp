 ...
 * @version $Id:  ImageController.java 697 2009-08-21 11:08:45Z oak021 $
 */
...
/**
 * Controls the display of ImageContentModel objects.
 *
 ...
 */
@Results({
    @Result(name="success", type="redirectAction", params = {"actionName" , "image"})
})
public class ImageController extends ActionSupport {
...
/**
 * List of all current images in the Fedora repository (made if no
 ...
 */
 ...
 //RELS-EXT properties
  ...
  private final boolean DEBUG_MSG = true;
   ...
   /**
   * Entry point to the controller from /AlaHarvester/image/<pid>
   * It displays a table of the images's properties

   * @return Forwards results to the image-show web page
   *
   * @since v0.1
   */
...
//next look for RELS-EXT properties
    ...
    try {
        ...
        List photos = (List) fcGetDs.findRELS_EXTValues("hasPhotoSourceUrl", RELS_EXTDataString);
            if (photos.size()>0) photoSourceUrl = (String) photos.get(0);
    } catch (ParserConfigurationException ex) {
   ...
   //now look for attributed properties
	...
	} // End of 'ImageController.show` method.

   /**
   * Entry point to the controller from /AlaHarvester/image/
   * It displays a list of all images in the repository

   * @return Forwards results to the image-index web page
   *
   * @since v0.1
   */
    public HttpHeaders index() {
        ...
        try {
            sr = (ArrayList<SearchResult>) PseudoRepository.doInitIndex("hasModel", "ala:ImageContentModel", null);
        } catch (Exception ex) {
