...
public class DocumentDAOImpl extends JdbcDaoSupport implements DocumentDAO {
	...
	protected static String INSERT = "insert into document " + 
			" (infosource_id,uri,file_path,mime_type,parent_document_id,created,modified) " + 
			" values (?,?,?,?,?,?,?)";

	protected static String INSERT_WITH_ID = "insert into document " + 
		" (infosource_id,uri,file_path,mime_type,parent_document_id,created,modified,id) " + 
		" values (?,?,?,?,?,?,?,?)";
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<Document> getByInfosourceId(int infosourceId) {
        List<Document> results = (List<Document>) getJdbcTemplate().query(
                //"select id, infosource_id, uri, file_path, mime_type from document where uri=?",
                "select d.id, d.infosource_id, ifs.name, ifs.uri, d.uri, d.file_path, d.mime_type from " +
                "document d INNER JOIN infosource ifs ON ifs.id = d.infosource_id where d.infosource_id=?",
    ...
    @Override
    public void changeInfosourceIdByDocumentId(final int documentId, final int infosourceId) {
        getJdbcTemplate().update(
                new PreparedStatementCreator() {
                    public PreparedStatement createPreparedStatement(Connection conn) throws SQLException {
                        PreparedStatement ps = conn.prepareStatement(
