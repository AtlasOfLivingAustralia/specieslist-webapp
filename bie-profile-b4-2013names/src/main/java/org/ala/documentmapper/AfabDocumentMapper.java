...
/**
 * Document mapper for Australian Fungi- A Blog
 *
 * @author Tommy Wang (Tommy.Wang@csiro.au)
 */
public class AfabDocumentMapper extends XMLDocumentMapper {

	/**
	 * Initialise the mapper, adding new XPath expressions
	 * for extracting content.
	 */
	public AfabDocumentMapper() {
        ...
        addDCMapping("//h3[@class=\"post-title entry-title\"]/text()",subject, Predicates.DC_TITLE);
        addTripleMapping("//h3[@class=\"post-title entry-title\"]/text()", subject, Predicates.SCIENTIFIC_NAME);
        addTripleMapping("//div[@class=\"post-body entry-content\"]", subject, Predicates.DESCRIPTIVE_TEXT);        
//        addTripleMapping("//td[@class=\"title\"]/text()", subject, Predicates.GENUS);
        addTripleMapping("//div[@class=\"post-body entry-content\"]//img/@src", subject, Predicates.IMAGE_URL);
	...
	protected void extractProperties(List<ParsedDocument> pds, Document xmlDocument) throws Exception {
		...
		for (Triple<String,String,String> triple: triples) {
			...
			if(predicate.endsWith("hasImageUrl")) {
                ...
                if(imageDoc!=null){
                    imageDoc.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "By Gaye Drady http://australianfungi.blogspot.com");
                    imageDoc.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Gaye Drady");
                    pds.add(imageDoc);
                }
			}
			...
			if(predicate.endsWith("hasScientificName")) {
			    ...
			    sciName = sciName.replaceAll("#[0-9]{1,}", "").trim();
