...
/**
 * Document mapper for Niels Klazenga 's Public Gallery
 *
 * @author Tommy Wang (Tommy.Wang@csiro.au)
 */
public class NkpgDocumentMapper extends XMLDocumentMapper {

	/**
	 * Initialise the mapper, adding new XPath expressions
	 * for extracting content.
	 */
	public NkpgDocumentMapper() {
...
//        addDCMapping("//meta[@name=\"descripiton\"]/text()",subject, Predicates.DC_TITLE);
        addTripleMapping("//link[@rel='image_src']/@href", subject, Predicates.IMAGE_URL);
        addTripleMapping("//meta[@name=\"description\"]/@content", subject, Predicates.SCIENTIFIC_NAME);
...
//		documentStr = documentStr.replaceAll("[\\s&&[^ ]]{0,}</em>[\\s&&[^ ]]{0,}", "");
	...
	protected void extractProperties(List<ParsedDocument> pds, Document xmlDocument) throws Exception {
		...
		pd.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "Copyright by Niels Klazenga");
		pd.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Niels Klazenga");
        ...
        for (Triple<String,String,String> triple: triples) {
            ...
            if(predicate.endsWith("hasScientificName")) {
                ...
                if (sciName.contains("-")) {
                    ...
                    sciName = sciName.split(",")[0].trim();
                    if (sciName != null && !"".equals(sciName)) {
                        triple.setObject(sciName);
		...
		for (Triple<String,String,String> triple: triples) {
			...
			if(predicate.endsWith("hasImageUrl")) {
                ...
                imageUrl = imageUrl.replaceAll("/s144/", "/s720/");
                ...
                if(imageDoc!=null){
                    imageDoc.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "Copyright by Niels Klazenga");
                    imageDoc.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Niels Klazenga");
                    pds.add(imageDoc);
                }
