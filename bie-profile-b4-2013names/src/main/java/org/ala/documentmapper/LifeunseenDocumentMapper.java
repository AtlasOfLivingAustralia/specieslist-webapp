...
/**
 * Document mapper for Life unseen
 *
 * @author Tommy Wang (Tommy.Wang@csiro.au)
 */
public class LifeunseenDocumentMapper extends XMLDocumentMapper {

	/**
	 * Initialise the mapper, adding new XPath expressions
	 * for extracting content.
	 */
	public LifeunseenDocumentMapper() {
...
//        addTripleMapping("//td[@class=\"title\"]/text()", subject, Predicates.GENUS);
        addTripleMapping("//table[@class=\"content_table\"]//img/@src", subject, Predicates.IMAGE_URL);
...
//		System.out.println(documentStr);
	...
	protected void extractProperties(List<ParsedDocument> pds, Document xmlDocument) throws Exception {
		...
		String source = "http://www.lifeunseen.com/";
		...
		pd.getDublinCore().put(Predicates.DC_LICENSE.toString(), "CC BY-NC Attribution - Non - Commercial 3.0 Australia");
		pd.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "Photo: Nick Monaghan, http://lifeunseen.com");
		pd.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Nick Monaghan");
		...
		for (Triple<String,String,String> triple: triples) {
			...
			if(predicate.endsWith("hasImageUrl")) {
                ...
                if(imageDoc!=null){
                    imageDoc.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "Photo: Nick Monaghan, http://lifeunseen.com");
                    imageDoc.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Nick Monaghan");
                    pds.add(imageDoc);
                }
			}
		}
