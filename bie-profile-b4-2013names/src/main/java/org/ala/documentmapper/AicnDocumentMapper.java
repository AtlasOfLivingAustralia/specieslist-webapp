...
public class AicnDocumentMapper extends XMLDocumentMapper {
    ...
    private void generateTitle(List<ParsedDocument> pds) {
        ...
        String title = (String) pd.getDublinCore().get(Predicates.DC_TITLE.toString());
        pd.getDublinCore().put(Predicates.DC_IDENTIFIER.toString(), "http://www.csiro.au/en/Organisation-Structure/National-Facilities/Australian-National-Insect-Collection.aspx");
        ...
        if (foundNumberingPrefix) {
            ...
            pd.getDublinCore().put(Predicates.DC_TITLE.toString(), titleParts[1].trim());
        }
    ...
    private void generateScientificName(List<ParsedDocument> pds) {
        ...
        String currentTitle = (String) pd.getDublinCore().get(Predicates.DC_TITLE);
    ...
    private void generateNamePartsFromSciName(List<ParsedDocument> pds) {
        ...
        if (sciName == null) {
            AicnDocumentMapper.classLogger.warn(
                    ...
                    " for data from " +
                    "`" + pd.getDublinCore().get(Predicates.DC_IDENTIFIER.toString()) + "`");
            return;
        }
    ...
    private void reformatImageUrl(List<ParsedDocument> pds) throws Exception {
        ...
        List<Triple<String,String,String>> triples = pd.getTriples();
        String subject = triples.get(0).getSubject();
        final String aicnBaseUrl = "http://www.ces.csiro.au/aicn/";

        //		pd.getDublinCore().put(Predicates.DC_CREATOR.toString(), "Australian Insect Common Names");
        ...
        if (!imageUrl.contains("noimage")) {
            ParsedDocument imageDoc = MappingUtils.retrieveImageDocument(pd,imageUrl);
            if(imageDoc!=null){
                imageDoc.getDublinCore().put(Predicates.DC_IDENTIFIER.toString(), "http://www.csiro.au/en/Organisation-Structure/National-Facilities/Australian-National-Insect-Collection.aspx");
                imageDoc.getDublinCore().put(Predicates.DC_IS_PART_OF.toString(), "http://www.csiro.au/en/Organisation-Structure/National-Facilities/Australian-National-Insect-Collection.aspx");
                pds.add(imageDoc);
            }
        }
