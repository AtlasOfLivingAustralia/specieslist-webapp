...
/**
 * Document mapper for Australian Succulents
 *
 * @author Tommy Wang (Tommy.Wang@csiro.au)
 */
public class AsDocumentMapper extends XMLDocumentMapper {

	/**
	 * Initialise the mapper, adding new XPath expressions
	 * for extracting content.
	 */
	public AsDocumentMapper() {
        ...
        addDCMapping("//ul[@class=\"picturelist\"]/preceding-sibling::h3[1]/text()",subject, Predicates.DC_TITLE);
        addTripleMapping("//li[@class=\"thumb\"]/a/@href", subject, Predicates.IMAGE_URL);
        addTripleMapping("//ul[@class=\"picturelist\"]/preceding-sibling::h3[1]/text()", subject, Predicates.SCIENTIFIC_NAME);
...
//		documentStr = documentStr.replaceAll("[\\s&&[^ ]]{0,}</em>[\\s&&[^ ]]{0,}", "");
	...
	protected void extractProperties(List<ParsedDocument> pds, Document xmlDocument) throws Exception {
		...
		pd.getDublinCore().put(Predicates.DC_LICENSE.toString(), "Attribution-Noncommercial-Share Alike - CC BY-NC-SA");
		...
		for (Triple<String,String,String> triple: triples) {
			...
			if(predicate.endsWith("hasImageUrl")) {
                ...
                if(imageDoc!=null){
                    imageDoc.getDublinCore().put(Predicates.DC_RIGHTS.toString(), "A&M. Kapitany, australiansucculents.com.au");
                    imageDoc.getDublinCore().put(Predicates.DC_CREATOR.toString(), "A&M. Kapitany");
                    pds.add(imageDoc);
                }
